...
exports.readConfig = function() {
  let config = {};
  try {
    config = YAML.readYaml('.redoclyrc');
  } catch (e) {
...
exports.syncWithSpec = function(spec, options = {}) {
  ...
  if (spec.openapi) {
    if (spec.components && dirExist(componentsDir)) {
      for (let componentType of OPENAPI3_COMPONENTS) {
...
exports.bundle = function(options = {}) {
  ...
  if (spec.openapi) {
    if (dirExist(componentsDir)) {
      if (spec.components) {
        throw Error(`All ${componentType} should be defined inside ${componentsDir}`);
      }
      spec.components = {};
...
exports.validate = function(spec, options = {}, cb) {
  if (spec.openapi) {
    ...
    var options = { prettify: false, lint: false, validateSchema: 'first' };
    let valid = false;
    try {
      valid = validator.validateSync(spec, options);
    } catch (e) {
