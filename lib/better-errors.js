...
module.exports = function(errors, paths, splitted) {
  ...
  for (let error of errors) {
    if (
      ...
      let correspondingRefError = errors.find(err => {
        return (
  ...
  errors = _(errors)
    ...
    .each(err => {
      if (allPaths && /^\/paths\/[^/]+/.test(err.dataPath)) {
        const [_em, _parent, specPath, ...rest] = err.dataPath.split('/');
        err.filePath = allPaths[pathToFilename(specPath) + '.yaml'];
        err.dataPath = rest.join('/');
        return;
      }
      if (/^\/components\/[^/]+\/[^/]+/.test(err.dataPath)) {
        const [_em, _parent, componentType, component, ...rest] = err.dataPath.split('/');
  ...
  let grouppedErrors = _.groupBy(errors, 'filePath');
  let errorNumber = 0;
  for (let filename of Object.keys(grouppedErrors)) {
    console.log(`${chalk.blue(filename)}:`);
    for (let error of grouppedErrors[filename]) {
